...
function [pure_jam] = generate_13npmj_jammingr(tx, params, data_num)
    ...
    for m = 1:data_num
        ...
        white_noise = white_noise / std(real(white_noise)); 
        ...
        if noise_distribution_mode == 1
            ...
            if rand() < 0.5 % 15% 的概率出现脉冲
                impulsive_noise = (randn([1,N_total]) + 1j*randn([1,N_total])) * 2; % 幅度放大3倍
                % 定向频域混合，使部分频段更脏
                % 这里简单线性叠加
