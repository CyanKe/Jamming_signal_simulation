...
% ==========================================================
function [pure_jam] = generate_3rgpo_jamming(tx, params, data_num)
% 解包参数
fs = params.fs;             % 采样率
...
for m = 1:data_num
    ...
    for p = 0:(Np-1)
        ...
        if jam_start_in_pri <= PRI_samp
            ...
            jam_signal(abs_start_idx:abs_end_idx) = Aj * lfm_template(1:valid_len);
            % 在 p 循环内部
            % doppler_freq = 2 * v / lambda; % lambda是雷达波长
            % phase_shift = exp(1j * 2 * pi * doppler_freq * (p * PRI_samp / fs));
            % jam_signal(abs_start_idx:abs_end_idx) = Aj * lfm_template * phase_shift;
        end
    end
