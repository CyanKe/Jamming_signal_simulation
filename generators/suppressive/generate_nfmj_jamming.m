function [pure_jam] = generate_nfmj_jamming(tx, params, data_num)
    % generate_nfmj_jamming - 生成噪声调频干扰(Noise Frequency Modulation Jamming, NPMJ) 
    % tx: 包含 LFM 信号的发射数据
    ...
    for m = 1:data_num
        ...
        white_noise = white_noise / std(white_noise); % 标准化，使功率归一化
        ...
        % --- 4. 生成噪声调频信号 ---
        % 公式: s(t) = Aj * exp(j * (2*pi*Fj*t + 2*pi*Kf * integral(m(t))))
        % 纯噪声调频信号
        carrier = exp(1j * 2 * pi * Fj * t);
        fm_jam_base = Aj * exp(1j * 2 * pi * Kf * phase_control / fs) .* carrier;
